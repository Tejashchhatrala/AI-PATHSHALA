    ...
    <div className="relative overflow-hidden bg-white pt-28 pb-20 lg:pt-36 lg:pb-32">
      ...
      <div className="container mx-auto px-4 sm:px-6 relative z-10">
        <div className="text-center max-w-5xl mx-auto">
          ...
          {/* Status Badge */}
          <div className="inline-flex items-center gap-3 px-4 py-2 rounded-full bg-slate-900 border border-slate-700 text-white shadow-lg mb-8 animate-fade-in-up cursor-default hover:scale-105 transition-transform">
             <span className="relative flex h-3 w-3">
              <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-red-400 opacity-75"></span>
              <span className="relative inline-flex rounded-full h-3 w-3 bg-red-500"></span>
            </span>
            <span className="text-xs md:text-sm font-bold tracking-wide uppercase">
              {lang === 'EN' ? 'Admissions Closing Soon' : 'પ્રવેશ પ્રક્રિયા ટૂંક સમયમાં બંધ થશે'}
            </span>
          </div>

          {/* Main Headline */}
          <h1 className="text-5xl md:text-7xl lg:text-8xl font-black text-slate-900 tracking-tight leading-[1.05] mb-8 drop-shadow-sm">
            {lang === 'EN' ? (
              <>
                ...
                Stop Memorizing.<br/>
                Start <span className="text-transparent bg-clip-text bg-gradient-to-r from-brand-600 via-brand-500 to-purple-600">Understanding</span>.
              </>
            ) : (
              <span className="font-gujarati leading-tight">
                ...
                ગોખણપટ્ટી છોડો,<br/>
                <span className="text-transparent bg-clip-text bg-gradient-to-r from-brand-600 via-brand-500 to-purple-600">AI સાથે ટોપર બનો</span>.
              </span>
            )}
          </h1>

          {/* Subheadline - QC: Changed "5 hours in 30 mins" to "Save 2 Hours" for realism */}
          ...
          {/* Buttons - ONLY CTA TO FORM */}
          <div className="mt-12 flex flex-col sm:flex-row gap-5 justify-center items-center">
            <div className="flex flex-col items-center gap-2 w-full sm:w-auto">
              <a 
                href="#enroll" 
                className="w-full sm:w-auto px-10 py-5 bg-brand-600 hover:bg-brand-700 text-white text-xl font-bold rounded-2xl shadow-xl shadow-brand-500/40 transition-all transform hover:-translate-y-1 hover:scale-105 flex items-center justify-center gap-3 border-t border-brand-400 ring-4 ring-brand-100"
              >
                {lang === 'EN' ? 'Book Free Roadmap Call' : 'ફ્રી રોડમેપ કોલ બુક કરો'}
                <ArrowRight className="w-6 h-6" />
              </a>
              <span className="text-[10px] md:text-xs font-bold text-slate-500 tracking-wide uppercase flex items-center gap-1">
                 <Lock className="w-3 h-3 text-brand-500 fill-current" />
                {lang === 'EN' ? 'Limited to 50 Students Only' : 'માત્ર 50 વિદ્યાર્થીઓ માટે મર્યાદિત'}
              </span>
            </div>
          </div>

          {/* Trust Indicators */}
           <div className="mt-16 pt-8 border-t border-slate-100 flex flex-wrap justify-center gap-x-8 gap-y-4 text-sm font-semibold text-slate-500">
          ...
          <div className="mt-12 p-6 bg-slate-50 rounded-2xl border border-slate-100 inline-block">
            <div className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-4">
               {lang === 'EN' ? "We Master The Top 5 Tools" : "અમે આ ટોપ 5 સાધનો શીખવીએ છીએ"}
            </div>
            <div className="flex flex-wrap justify-center items-center gap-4 md:gap-8">
               {['ChatGPT', 'Gemini', 'Claude', 'Perplexity', 'Gamma'].map(tool => (
                 <span key={tool} className="text-lg md:text-xl font-black text-slate-700 hover:text-brand-600 transition-colors cursor-default">{tool}</span>
               ))}
            </div>
          </div>
