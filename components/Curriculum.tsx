...
import { Flag, Compass, MessageSquare, Trophy, Briefcase } from 'lucide-react';
    ...
    <section id="curriculum" className="py-24 bg-white relative overflow-hidden">
      ...
      <div className="container mx-auto px-4 sm:px-6 relative z-10">
        <div className="text-center mb-16">
          ...
          <h2 className={`text-3xl md:text-5xl font-black text-slate-900 mt-6 mb-6 leading-tight ${lang === 'GU' ? 'font-gujarati' : ''}`}>
        ...
        <div className="max-w-3xl mx-auto">
          <div className="relative">
            {/* Vertical Line */}
            <div className="absolute left-8 md:left-1/2 top-0 bottom-0 w-1 bg-slate-100 transform md:-translate-x-1/2"></div>
            ...
            <div className="space-y-12">
              {phases.map((phase, idx) => (
                <div key={idx} className={`relative flex items-center md:justify-between ${idx % 2 === 0 ? 'md:flex-row' : 'md:flex-row-reverse'}`}>
                  ...
                  {/* Content Box */}
                  <div className={`ml-24 md:ml-0 md:w-[42%] bg-white p-6 rounded-2xl border border-slate-100 shadow-lg hover:shadow-xl transition-shadow ${idx % 2 === 0 ? 'md:text-right' : 'md:text-left'}`}>
                     ...
                     <p className={`text-slate-500 text-sm leading-relaxed ${lang === 'GU' ? 'font-gujarati' : ''}`}>
