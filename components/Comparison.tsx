    ...
    <section className="py-24 bg-white">
      <div className="container mx-auto px-4 sm:px-6">
        <div className="text-center mb-16">
          <h2 className={`text-3xl md:text-5xl font-black text-slate-900 mb-6 ${lang === 'GU' ? 'font-gujarati' : ''}`}>
          ...
          <table className="w-full max-w-5xl mx-auto border-collapse">
            <thead>
              <tr className="border-b-2 border-slate-100">
                ...
                <th className="p-6 text-center w-1/3 text-slate-400 font-medium text-lg bg-slate-50 rounded-t-2xl">
                   {lang === 'EN' ? "Traditional Tuition" : "સામાન્ય ટ્યુશન"}
                </th>
                <th className="p-6 text-center w-1/3 text-brand-600 font-bold text-xl bg-brand-50 rounded-t-2xl border-t-4 border-brand-500">
                   AI Pathshala
                </th>
              </tr>
            </thead>
            <tbody>
              {[
                ...
                <tr key={i} className="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                  <td className={`p-6 text-slate-800 font-bold text-lg ${lang === 'GU' ? 'font-gujarati' : ''}`}>
                    {lang === 'EN' ? row.feat.en : row.feat.gu}
                  </td>
                  <td className={`p-6 text-center text-slate-500 bg-slate-50/50 ${lang === 'GU' ? 'font-gujarati' : ''}`}>
                    <div className="flex items-center justify-center gap-2">
                       <X className="w-5 h-5 text-red-400" />
                       {lang === 'EN' ? row.old.en : row.old.gu}
                    </div>
                  </td>
                  <td className={`p-6 text-center font-bold text-brand-700 bg-brand-50/30 ${lang === 'GU' ? 'font-gujarati' : ''}`}>
                    <div className="flex items-center justify-center gap-2">
